// svc-01-ingestion entrypoint — receives raw emails from external sources
// and enqueues them for svc-02-parser.
package main
...
func main() {
	...
	tracerShutdown, err := tracing.Init(ctx, "svc-01-ingestion", cfg.JaegerEndpoint)
	if err != nil {
	...
	reg := metrics.Init("svc-01-ingestion")
	...
	log.Info().
		...
		Msg("svc-01-ingestion started")

	// TODO: initialize email ingestion from external sources (IMAP, webhook, etc.)
	// TODO: enqueue raw email messages for svc-02-parser
