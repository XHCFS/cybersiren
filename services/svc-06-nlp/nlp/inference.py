...
so the numeric risk is phishing-specific; benign marketing spam scores ~0 (it is not a
phishing threat and must not pollute the score), while advance-fee scams are phishing-
labelled and stay high. The 3-class label and spam_probability still distinguish phishing
vs spam vs legitimate. See benchmark/FINDINGS.md. URLs are stripped before tokenization —
their reputation is SVC-03's
...
class NLPInferenceEngine:
    ...
    def predict(
        ...
        # 5. Scoring scheme (v4): content risk = P(phishing). The aggregator (SVC-08)
        #    detects PHISHING (the verdict pipeline has no automated spam label), so the
        #    numeric risk it consumes must be phishing-specific. v3 scored maliciousness
        #    (1 - P(legitimate) = P(spam) + P(phishing)) to stop scams passing as benign,
        #    but that conflated benign MARKETING spam with threats: measured on the
        #    consistent real-model benchmark it scored spam ~85, polluting the negative
        #    distribution and capping phishing recall@1%FPR at ~58% (worse than NLP alone).
        #    Advance-fee / 419 SCAMS — the cases v3 worried about — are labelled *phishing*
        #    and score high under P(phishing), so they remain caught; only benign marketing
        #    spam drops to ~0, which is correct (it is not a phishing threat). Switching the
        #    composite to P(phishing) lifts whole-system held-out recall@2%FPR to ~96%
        #    (ROC-AUC ~0.995). See benchmark/FINDINGS.md. The 3-class label and
        #    spam_probability below are unchanged, so callers that want a spam signal keep it.
