...
// DSL design
// ----------------------------------------------------------------------------
// rules.logic is a JSONB column. SVC-04 needs to evaluate it against a
// flat snapshot of header signals. To keep the surface area small while
// still being expressive enough for the seed rules, we accept this
// minimal grammar:
//
...
//
// Recognised signal identifiers are listed in the SignalSnapshot keys
// below. Unknown signals or operators short-circuit to "no match" and
// log at debug — we do NOT throw, in line with ARCH-SPEC §6 ("skip
// malformed rule, log, continue").
//
...
// Keys are stable — they're part of the rule contract and can be
...
func Evaluate(logic json.RawMessage, signals SignalSnapshot) (MatchResult, error) {
	...
	if catRaw, ok := root["category"]; ok {
		...
		// When a "category" key is present, "expr" carries the actual
		// condition. Everything else (signal/op/value, all/any/not) is
		// nested inside it.
		var ok bool
...
func parseCategory(raw string) Category {
	...
	default:
		// Default to auth so a rule with an unknown category still
		// contributes deterministically to one bucket.
		return CategoryAuth
	}
}
...
func equalValues(a, b any) bool {
	...
	// Numeric comparisons: JSON unmarshals every number as float64.
	an, aOK := toFloat(a)
	bn, bOK := toFloat(b)
	if aOK && bOK {
		return an == bn
	...
	// Slices and maps: compare via JSON equality.
	ab, _ := json.Marshal(a)
	bb, _ := json.Marshal(b)
