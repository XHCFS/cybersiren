// svc-02-parser entrypoint — consumes raw email messages and emits
// structured parsed-email events for downstream analysis services.
package main
...
func main() {
	...
	tracerShutdown, err := tracing.Init(ctx, "svc-02-parser", cfg.JaegerEndpoint)
	if err != nil {
	...
	reg := metrics.Init("svc-02-parser")
	...
	log.Info().
		...
		Msg("svc-02-parser started")

	// TODO: consume raw email messages from svc-01-ingestion queue
	// TODO: parse MIME structure, extract headers, body, attachments
	// TODO: emit structured parsed-email events for downstream analysis services
