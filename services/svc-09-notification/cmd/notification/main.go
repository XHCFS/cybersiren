// svc-09-notification entrypoint — subscribes to verdict events and
// dispatches alerts (email, webhook, Slack) to org administrators.
package main
...
func main() {
	...
	tracerShutdown, err := tracing.Init(ctx, "svc-09-notification", cfg.JaegerEndpoint)
	if err != nil {
	...
	reg := metrics.Init("svc-09-notification")
	...
	log.Info().
		...
		Msg("svc-09-notification started")

	// TODO: subscribe to verdict events from svc-08-decision
	// TODO: dispatch alerts (email, webhook, Slack) to org administrators
